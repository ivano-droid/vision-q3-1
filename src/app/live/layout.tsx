...
import { LiveSubNav } from "@/components/LiveSubNav";
import { getLiveSectionBySlug } from "@/lib/live-sections";
...
function tabIndexFor(pathname: string): number {
  if (pathname === "/live") return 0;
  if (pathname.startsWith("/live/section/")) return 99;
  const i = TAB_PATHS.findIndex((t, idx) => idx > 0 && pathname.startsWith(t));
  return i >= 0 ? i : 0;
}

function titleFor(pathname: string): string {
  if (pathname.startsWith("/live/section/")) {
    const slug = pathname.split("/live/section/")[1]?.split("/")[0] ?? "";
    return getLiveSectionBySlug(slug)?.title ?? "Live Casino";
  }
  return "Live Casino";
}
...
export default function LiveLayout({ children }: { children: ReactNode }) {
  ...
  const isSection = pathname.startsWith("/live/section/");
